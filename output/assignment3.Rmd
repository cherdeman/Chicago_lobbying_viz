...
contribution %>% filter(str_detect(RECIPIENT, 'REILLY')) %>% arrange(desc(CONTRIBUTION_DATE))

alderman_contributions %>% 
  filter(ALDERMAN == 'BURKE' & CONTRIBUTION_DATE > as.Date('2014-12-31')) %>% 
  group_by(LOBBYIST_NAME) %>% 
  summarise(sum = sum(AMOUNT)) %>% arrange(desc(sum))
  ggplot(aes(month, sum, color = year)) +
  geom_point()
  
alderman_contributions %>% 
  mutate(paid_top20 = if_else(LOBBYIST_NAME %in% paid_by_top20$LOBBYIST_NAME | LOBBYIST_NAME == 'REYES KURSON', TRUE, FALSE),
         YEAR = format(CONTRIBUTION_DATE, '%Y')) %>% 
  filter(CONTRIBUTION_DATE > as.Date('2014-12-31') & paid_top20) %>% 
  group_by(LOBBYIST_NAME, ALDERMAN, YEAR) %>% summarise(sum = sum(AMOUNT)) %>% 
    ggplot(aes(y = sum, axis1 = LOBBYIST_NAME, axis2 = ALDERMAN)) +
...
lobbyist_name <- c("VICTOR REYES", "AMY KURSON", "PATRICK CAREY", "JOHN DUNN", "JOHN KELLY, JR.", 
                   ...
                   "MICHAEL KASPER")
lawfirm <- c("REYES KURSON", "REYES KURSON", "MCGUIRE WOODS", "MCGUIRE WOODS", "ALLCIRCO", 
             "FLETCHER, O'BRIEN, KASPER, AND NOTTAGE", "FLETCHER, O'BRIEN, KASPER, AND NOTTAGE", 
             "FLETCHER, O'BRIEN, KASPER, AND NOTTAGE")
...
compensation %>% 
  ...
  group_by(CLIENT_INDUSTRY, LAWFIRM) %>% 
  summarise(sum = sum(COMPENSATION_AMOUNT)) %>% 
