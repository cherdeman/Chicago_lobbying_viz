...
```{r graph2, warning=FALSE, error=FALSE}
compensation %>% 
  filter(TOP20 == 'Yes') %>% 
  ...
  mutate(sum = sum/1000000,
         INDUSTRY = if_else(CLIENT_INDUSTRY %in% c('HOSPITALITY / RESTAURANT', 'PUBLIC UTILITIES', 
                                                   'RETAIL', 'EDUCATION', 'RACING & WAGERING'), 
  ...
  ggplot() +
    geom_point(aes(n, sum, color = INDUSTRY), size = 2, alpha = 0.7) +
    geom_text_repel(aes(n, sum, label = NICK_NAME), check_overlap = TRUE, size = 2, hjust = 0, nudge_x = 0.1) + #vjust = "inward",
    scale_y_continuous(label=dollar_format()) +
    xlim(0, 650) +
...
```{r councilmen, warning=FALSE, error=FALSE}
...
alderman_contributions %>% 
  ...
  arrange(desc(sum)) %>%
  ggplot() +
  geom_col(aes(ALDERMAN, sum, fill=sum)) + coord_flip()

```

```{r}
alderman_contributions %>% filter(str_detect(RECIPIENT, "O'CONNOR")) %>% group_by(LOBBYIST_NAME) %>% summarise(n = n()) %>% arrange(desc(n))
```
