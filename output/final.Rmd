...
t <-compensation %>% 
  ...
  annotate("rect", xmin = 275, xmax = 425, ymin = 2, ymax = 3, fill = light_colors['gray'], alpha = .2) 
...
yes <-  data.frame(table(top20cloud$WORDS_SEP[1])) %>%
  rename(WORD = Var1, COUNT = Freq) %>%
  filter(!(WORD %in% common_words)) %>%
  arrange(desc(COUNT)) %>%
  slice(1:50) %>% 
  mutate(norm = COUNT/max(COUNT),
         TOP20 = "Y")
no <- data.frame(table(top20cloud$WORDS_SEP[2])) %>%
  rename(WORD = Var1, COUNT = Freq) %>%
  filter(!(WORD %in% common_words)) %>%
  arrange(desc(COUNT)) %>%
  slice(1:50) %>% 
...
alderman_contributions %>% 
  ...
  ggplot(aes(y = sum, axis1 = reorder(LOBBYIST_NAME, sum), axis2 = reorder(ALDERMAN, -sum))) +
