...
c <- compensation %>% 
  ...
  group_by(TOP20, PERIOD_START, year) %>%
  ...
  ggplot(aes(PERIOD_START, sum)) +
    ...
    scale_x_yearqtr(breaks = seq(from = 2012, to = 2018.75, by = 0.25), format = "Q%q") +
    scale_fill_manual(values=c(as.vector(light_colors['blue']), as.vector(dark_colors['blue']))) + 
    #annotate(geom = "text", x = 2012.5 + 1 * (0:6), y = -1, 
    #         label = c("2012", "2013", "2014", "2015", "2016", "2017", "2018"), size = 3) +
...
t <-compensation %>% 
  ...
  ggplot() +
    ...
    annotate("text", x = 300, y = .7, label = "99% of clients paid under\n $2.5M to lobbyists\n in the time period", size = 3)
