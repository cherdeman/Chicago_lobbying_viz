...
```{r graph}
c('LIGHTFOOT', 'MENDOZA', 'PRECKWINKLE', 'CHICO') 

contribution %>% 
  filter(str_detect(RECIPIENT, 'LIGHTFOOT') |
         str_detect(RECIPIENT, 'MENDOZA') |
         str_detect(RECIPIENT, 'PRECKWINKLE') |
         str_detect(RECIPIENT, 'CHICO')) %>%
  mutate(candidate = if_else(str_detect(RECIPIENT, 'LIGHTFOOT'), 'LIGHTFOOT',
                             if_else(str_detect(RECIPIENT, 'MENDOZA'), 'MENDOZA',
                                     if_else(str_detect(RECIPIENT, 'PRECKWINKLE'), 'PRECKWINKLE',
                                             if_else(str_detect(RECIPIENT, 'CHICO'), 'CHICO', NULL))))) %>%
  group_by(candidate) %>% 
  summarise(sum=sum(AMOUNT), n = n()) %>%
  ggplot() +
    geom_point(aes(n, sum, size = sum), color = "blue") +
    scale_size(range=c(1,20)) +
    labs(title="Of the Subset of Candidates",
         subtitle="Total Contributions to Current Mayoral Candidates, 2012 - 2018",
         caption="Source: City of Chicago Lobbyist Data, Contributions",
         x = "Number of Contributions",
         y = "Total Amount of Contributions")
```
...
compensation %>%
  ...
  arrange(desc(sum)) %>%
  slice(1:20) %>% 
  ggplot(aes(reorder(CLIENT_NAME, -sum), sum)) +
    geom_col() + 
    theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
# Top compensation by client over time 
compensation %>%
  group_by(CLIENT_NAME, year = (format(PERIOD_START, '%Y'))) %>%
  summarise(sum = sum(COMPENSATION_AMOUNT), cnt = n()) %>%
  ggplot(aes(year, sum, group = CLIENT_NAME)) +
    #geom_area(position = 'stack')
    geom_line(aes(group = CLIENT_NAME))

# Top 20 compensation to lobbyists by client, 2018
compensation %>%
  filter(PERIOD_START > as.Date('2017-12-31') & PERIOD_END < as.Date('2019-01-01')) %>%
  group_by(CLIENT_NAME) %>%
  summarise(sum = sum(COMPENSATION_AMOUNT), cnt = n()) %>%
  arrange(desc(sum)) %>%
  slice(1:20) %>% 
...
# Top 20 compensation by lobbyists, 2018
compensation %>%
  filter(PERIOD_START > as.Date('2017-12-31') & PERIOD_END < as.Date('2019-01-01')) %>%
...
# Top compensation by lobbyist over time 
compensation %>%
  group_by(LOBBYIST_NAME, year = (format(PERIOD_START, '%Y'))) %>%
  summarise(sum = sum(COMPENSATION_AMOUNT), cnt = n()) %>%
  ggplot(aes(year, sum)) +
    geom_line(aes(group = LOBBYIST_NAME))

# Top 20 compensation to lobbyists by industry, 2018
compensation %>%
  filter(PERIOD_START > as.Date('2017-12-31') & PERIOD_END < as.Date('2019-01-01')) %>%
  group_by(CLIENT_INDUSTRY) %>%
  summarise(sum = sum(COMPENSATION_AMOUNT), cnt = n()) %>%
  arrange(desc(sum)) %>%
  ggplot(aes(reorder(CLIENT_INDUSTRY, -sum), sum)) +
    geom_col() + 
    theme(axis.text.x = element_text(angle = 90, hjust = 1))

# Compensation for other/nas

compensation %>% filter(is.na(CLIENT_INDUSTRY)) %>% group_by(CLIENT_NAME) %>% summarise(sum = sum(COMPENSATION_AMOUNT), n=n()) %>% arrange(desc(sum))
compensation %>% filter(CLIENT_INDUSTRY == 'OTHER')
```
...
```{r contributions}
contribution %>% 
  filter(CONTRIBUTION_DATE > as.Date('2017-12-31') & CONTRIBUTION_DATE < as.Date('2019-01-01')) %>%
  group_by(RECIPIENT) %>%
  summarise(sum = sum(AMOUNT), cnt = n()) %>%
  arrange(desc(sum))
...
contribution %>% 
  group_by(RECIPIENT, year = (format(PERIOD_START, '%Y')))%>%
  summarise(sum = sum(AMOUNT), cnt = n()) %>%
  ggplot(aes(year, sum)) +
    geom_boxplot()
...
contribution %>% 
  filter(CONTRIBUTION_DATE > as.Date('2017-12-31') & CONTRIBUTION_DATE < as.Date('2019-01-01')) %>%
  group_by(LOBBYIST_NAME, RECIPIENT)%>%
  summarise(sum = sum(AMOUNT), cnt = n()) %>%
  arrange(desc(sum))
...
activity %>% 
  ggplot() +
  geom_count(aes(DEPARTMENT, ACTION)) + 
    theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
activity %>% 
  filter(PERIOD_START > as.Date('2017-12-31') & PERIOD_END < as.Date('2019-01-01')) %>%
  ggplot() +
  geom_count(aes(CLIENT_INDUSTRY, ACTION))+ 
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    coord_flip()

```

# 2018 top compensation client

```{r ups}
compensation %>%
  filter(CLIENT_NAME == 'United Parcel Service') %>% 
  group_by(LOBBYIST_NAME, PERIOD_START) %>%
  summarise(total_comp = sum(COMPENSATION_AMOUNT), n = n())
```

# Top Paid Lobbyist of All time 
```{r}
compensation %>% 
  group_by(LOBBYIST_NAME) %>% 
  summarise(sum = sum(COMPENSATION_AMOUNT), n = n()) %>% 
...
lobbyists %>% 
  distinct(CLIENT_NAME, TRUE) %>%
  ggplot() +
  geom_count(aes(EMPLOYER_NAME, LOBBYIST_NAME))
...
- Compensation and/or gifts over time (by industry or client)? line chart, or how much each industry/client invest in lobbying over time
- Volume of meetings by department - bubble chart?
- compensaton/contribution date scatter plot - are there spikes? or histogram
- comparison of total pay of lobbyists in time period by lobbyist

```{r change}
comp_pct <- compensation %>%
              filter(PERIOD_START > as.Date('2012-06-30')) %>% #& PERIOD_START < as.Date('2018-07-01')
              group_by(PERIOD_START) %>%
              summarise(sum = sum(COMPENSATION_AMOUNT), n = n()) %>%
              mutate(sum = sum/1000000,
                     pct_change = (sum - lag(sum))/sum * 100)

cont_pct <- contribution %>%
              filter(PERIOD_START > as.Date('2012-06-30')) %>% # & PERIOD_START < as.Date('2018-07-01')
              group_by(PERIOD_START) %>%
              summarise(sum = sum(AMOUNT), n = n()) %>%
              mutate(sum = sum/1000000,
...
"""ggplot(NULL, aes(PERIOD_START, pct_change)) +
  geom_line(data = comp_pct, color = 'blue', alpha = 0.7) +
  geom_line(data = cont_pct, color = 'red', alpha = 0.7) +
  geom_hline(aes(yintercept = 0), alpha = 0.5) +
  geom_vline(aes(xintercept = as.Date('2013-01-01')), color = 'gray') +
  geom_vline(aes(xintercept = as.Date('2014-01-01')), color = 'gray') +
  geom_vline(aes(xintercept = as.Date('2015-01-01')), color = 'gray') +
  geom_vline(aes(xintercept = as.Date('2016-01-01')), color = 'gray') +
  geom_vline(aes(xintercept = as.Date('2017-01-01')), color = 'gray') +
  geom_vline(aes(xintercept = as.Date('2018-01-01')), color = 'gray') +
  xlim(as.Date('2012-07-01'), as.Date('2019-01-01')) +
  ylim(-600, 400)  +
  scale_x_date(date_breaks='3 months') + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1))"""

comp_pct %>% ggplot() + geom_col(aes(PERIOD_START, sum))
cont_pct %>% ggplot() + geom_col(aes(PERIOD_START, sum))
